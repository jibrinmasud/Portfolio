...
const projcts =[
  ...
  {id:3, img:studentrec, title:" C# StudentRecords ", liveUrl:'/'},
]

const Work = () => {
  return (
    <div className='glass mx-auto grid grid-cols-2 gap-4 mt-10 p-4 max-w-[1200px]  relative' id='work'>

      <div className='col-span-2 my-4'>
<h1 className='text-4xl font-semibold border-b-4 border-gray-500 p-2 inline text-white'>Portfolio</h1>
      </div>
{projcts.map((project)=>(
  <div  key={project.id} className='glass transform transition-transform hover:scale-110 grid-cols-1 sm:grid-cols-3 gap-10 duration-300 overflow-hidden shadow-lg h-[200px] group relative'>
    ...
    <div className='group-hover:opacity-90 opacity-0 bg-[#232323]  absolute transition-opacity duration-300 inset-0 flex  flex-col justify-center items-center'>
      <span className='text-1xl text-center font-bold text-white mb-4'>{project.title}</span>
      <div>
